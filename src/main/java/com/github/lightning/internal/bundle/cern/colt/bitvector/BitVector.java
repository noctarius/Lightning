 ...
 */
package com.github.lightning.internal.bundle.cern.colt.bitvector;

/**
 * Fixed sized (non resizable) bitvector.
 * Upon instance construction a bitvector is told to hold a fixed number of bits
 * - it's size.
 ...
 * Any attempt to access a bit at an <tt>index&lt;0 || index&gt;=size()</tt>
 ...
 */
public class BitVector extends com.github.lightning.internal.bundle.cern.colt.PersistentObject {
	...
	private class IndexProcedure implements com.github.lightning.internal.bundle.cern.colt.function.IntProcedure {
	 ...
	 *             ).
	 */
	public boolean forEachIndexFromToInState(int from, int to, boolean state, com.github.lightning.internal.bundle.cern.colt.function.IntProcedure procedure) {
		/*
		 * // this version is equivalent to the low level version below, but
		 * about 100 times slower for large ranges.
		 ...
		 * unit one can determine in O(1)
		 * whether it contains at least one true bit,
		 * and whether it contains at least one false bit.
		 * Thus, 64 bits can often be skipped with one simple comparison if the
		 * vector is either sparse or dense.
		 * 
	 ...
	 * <tt>from</tt> to index <tt>to</tt>.
	 * Bits are returned as a long value with the return value having bit 0 set
	 * to bit <code>from</code>, ..., bit <code>to-from</code> set to bit
	 * <code>to</code>.
	 ...
	 * state. Returns <code>-1</code> if the receiver does not contain this
	 * state.
	 ...
	 * <tt>[sourceFrom,sourceFrom+to-from]</tt>, all inclusive.
	 * If <tt>source==this</tt> and the source and destination range intersect
	 * in an ambiguous way, then replaces as if using an intermediate auxiliary
	 ...
	 * the end.
	 * If the receiver is shrinked, all bits between the old size and the new
	 * size are lost; their memory is subject to garbage collection.
	 ...
	 * = A ^ B).
	 * The receiver is modified so that a bit in it has the
	 * value <code>true</code> if and only if one of the following statements
	 * holds:
	 * <ul>
