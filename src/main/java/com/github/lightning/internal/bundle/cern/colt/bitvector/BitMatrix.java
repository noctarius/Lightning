 ...
 * A bit matrix has a number of columns and rows, which are assigned upon
 * instance construction - The matrix's size is then <tt>columns()*rows()</tt>.
 ...
 */
public class BitMatrix extends com.github.lightning.internal.bundle.cern.colt.PersistentObject {
	...
	protected void checkDimensionCompatibility(BitMatrix other) {
		if (columns != other.columns() || rows != other.rows())
			throw new IllegalArgumentException("Incompatible dimensions: (columns,rows)=(" + columns + "," + rows + "), (other.columns,other.rows)=(" + other.columns() + ","
					+ other.rows() + ")");
	}
	 ...
	 */
	public boolean forEachCoordinateInState(boolean state, final com.github.lightning.internal.bundle.cern.colt.function.IntIntProcedure procedure) {
		/*
		 * this is equivalent to the low level version below, apart from that it
		 * iterates in the reverse oder and is slower.
	 ...
	 * coordinate.
	 * The value is <tt>true</tt> if this bit is currently set; otherwise,
	 * returns <tt>false</tt>.
	 * 
	 * @param column
	...
	/**
	 * Returns from the receiver the value of the bit at the specified
	 * coordinate; <b>WARNING:</b> Does not check preconditions.
	 * The value is <tt>true</tt> if this bit is currently set; otherwise,
	 * returns <tt>false</tt>.
	 * 
	 * <p>
	 ...
	 * <tt>[sourceColumn+width-1,sourceRow+height-1]</tt>, all inclusive.
	 * The destination box ranges from <tt>[column,row]</tt> to
	 * <tt>[column+width-1,row+height-1]</tt>, all inclusive.
	 * Does nothing if <tt>width &lt;= 0 || height &lt;= 0</tt>.
	 * If <tt>source==this</tt> and the source and destination box intersect in
	 * an ambiguous way, then replaces as if using an intermediate auxiliary
	 ...
	 * <b>WARNING:</b> The returned bitvector and the receiver share the
	 * <b>same</b> backing bits.
	 * Modifying either of them will affect the other.
	 * If this behaviour is not what you want, you should first use
	 * <tt>copy()</tt> to make sure both objects use separate internal storage.
	 */
