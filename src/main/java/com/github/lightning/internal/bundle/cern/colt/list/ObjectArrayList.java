 ...
 */
package com.github.lightning.internal.bundle.cern.colt.list;

import com.github.lightning.internal.bundle.cern.colt.function.ObjectProcedure;
...
public class ObjectArrayList extends AbstractList {
	 ...
	 * the binary search algorithm. The receiver must be sorted into ascending
	 * order
	 * according to the <i>natural ordering</i> of its elements (as by the sort
	 * method)
	 ...
	 *             if the receiver contains elements that are not
	 ...
	 *         that this guarantees that the return value will be &gt;= 0 if
	 *         and only if the key is found.
	 * @see com.github.lightning.internal.bundle.cern.colt.Sorting
	 * @see java.util.Arrays
	 * @see java.util.Comparator
	 */
	public int binarySearchFromTo(Object key, int from, int to, java.util.Comparator comparator) {
		return com.github.lightning.internal.bundle.cern.colt.Sorting.binarySearchFromTo(this.elements, key, from, to, comparator);
	}
	 ...
	 * them;
	 * So modifying an object in the copy modifies the object in the receiver
	 * and vice versa;
	 * However, structurally modifying the copy (for example changing its size,
	 * setting other objects at indexes, etc.) does not affect the receiver and
	 * vice versa.
	 * 
	 ...
	 * result.
	 * Returns a copy such that the copy and the receiver <i>share</i> the same
	 * elements, but do not share the same array to index them;
	 * So modifying an object in the copy modifies the object in the receiver
	 * and vice versa;
	 * However, structurally modifying the copy (for example changing its size,
	 * setting other objects at indexes, etc.) does not affect the receiver and
	 ...
	 * <tt>testForEquality</tt>.
	 * When testing for equality, two elements <tt>e1</tt> and <tt>e2</tt> are
	 ...
	 * If necessary, allocates new internal memory and increases the capacity of
	 * the receiver.
	 * 
	 * @param minCapacity
	...
	public void ensureCapacity(int minCapacity) {
		elements = com.github.lightning.internal.bundle.cern.colt.Arrays.ensureCapacity(elements, minCapacity);
	}

	/**
	 * Compares the specified Object with the receiver for equality.
	 * Returns true if and only if the specified Object is also an
	 * ObjectArrayList, both lists have the
	 ...
	 * element. Returns <code>-1</code> if the receiver does not contain this
	 * element.
	 * 
	 * Tests for equality or identity as specified by testForEquality.
	 * 
	...
	public void quickSortFromTo(int from, int to) {
		if (size == 0)
			return;
		checkRangeFromTo(from, to, size);
		com.github.lightning.internal.bundle.cern.colt.Sorting.quickSort(elements, from, to + 1);
	}
	...
	public void quickSortFromTo(int from, int to, java.util.Comparator c) {
		if (size == 0)
			return;
		checkRangeFromTo(from, to, size);
		com.github.lightning.internal.bundle.cern.colt.Sorting.quickSort(elements, from, to + 1, c);
	}
	 ...
	 * Replaces elements in the receiver, between <code>from</code> (inclusive)
	 * and <code>to</code> (inclusive),
	 * with elements of <code>other</code>, starting from <code>otherFrom</code>
	 * (inclusive).
	 * 
	 * @param from
	 ...
	 * <code>to</code> (inclusive) with the other list's
	 * part between <code>otherFrom</code> and <code>otherTo</code>.
	 * Powerful (and tricky) method!
	 * Both parts need not be of the same size (part A can both be smaller or
	 * larger than part B).
	 * Parts may overlap.
	 * Receiver and other list may (but most not) be identical.
	 * If <code>from &gt; to</code>, then inserts other part before
	 * <code>from</code>.
	 * 
	 * @param from
	 ...
	 * Replaces exactly <tt>Math.max(0,Math.min(size()-from, other.size()))</tt>
	 * elements.
	 * 
	 * @param from
	 ...
	 * In other words, removes from the receiver all of its elements that are
	...
	public void shuffleFromTo(int from, int to) {
		...
		com.github.lightning.internal.bundle.cern.jet.random.Uniform gen = new com.github.lightning.internal.bundle.cern.jet.random.Uniform(
				new com.github.lightning.internal.bundle.cern.jet.random.engine.DRand(new java.util.Date()));
		Object tmpElement;
		Object[] theElements = elements;
		int random;
		for (int i = from; i < to; i++) {
	 ...
	 * 
	 * @param array
	 *            the array into which the elements of the receiver are to
	 *            be stored, if it is big enough; otherwise, a new array of the
	 *            same runtime type is allocated for this purpose.
	 * @return an array containing the elements of the receiver.
	 * @exception ArrayStoreException
	 *                the runtime type of <tt>array</tt> is not a supertype
	...
	public String toString() {
		return com.github.lightning.internal.bundle.cern.colt.Arrays.toString(partFromTo(0, size() - 1).elements());
	}

	/**
	 * Trims the capacity of the receiver to be the receiver's current
	 * size. Releases any superfluos internal memory. An application can use
	 * this operation to minimize the
	 * storage of the receiver.
	 */
	@Override
	public void trimToSize() {
		elements = com.github.lightning.internal.bundle.cern.colt.Arrays.trimToCapacity(elements, size());
	}
}
