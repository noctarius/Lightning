...
import com.github.lightning.internal.bundle.cern.colt.function.FloatComparator;
import com.github.lightning.internal.bundle.cern.colt.function.FloatProcedure;
...
public abstract class AbstractFloatList extends AbstractList {
	 ...
	 * !!!
	 * If you violate this principle in subclasses, you should exactly know what
	 * you are doing.
	 * 
	 * @serial
	 */
	 ...
	 * Returns true if and only if the specified Object is also an ArrayList of
	 * the same type, both Lists have the
	 ...
	 * the loop.
	 * However, when desperately, you can give this method <tt>public</tt>
	...
	public void mergeSortFromTo(int from, int to) {
		...
		com.github.lightning.internal.bundle.cern.colt.Sorting.mergeSort(myElements, from, to + 1);
		elements(myElements);
		setSizeRaw(mySize);
	}
	...
	public void mergeSortFromTo(int from, int to, FloatComparator c) {
		...
		float[] myElements = elements();
		com.github.lightning.internal.bundle.cern.colt.Sorting.mergeSort(myElements, from, to + 1, c);
		elements(myElements);
		setSizeRaw(mySize);
	}
	...
	public void quickSortFromTo(int from, int to, FloatComparator c) {
		...
		float[] myElements = elements();
		com.github.lightning.internal.bundle.cern.colt.Sorting.quickSort(myElements, from, to + 1, c);
		elements(myElements);
		setSizeRaw(mySize);
	}
	 ...
	 * This method should not release or allocate new memory but simply set some
	 * instance variable like <tt>size</tt>.
	 * 
	 * If your subclass overrides and delegates size changing methods to some
	 * other object,
	 * you must make sure that those overriding methods not only update the size
	 * of the delegate but also of this class.
	 * For example:
