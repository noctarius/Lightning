...
public class LongArrayList extends AbstractLongList {
	...
	public int binarySearchFromTo(long key, int from, int to) {
		return com.github.lightning.internal.bundle.cern.colt.Sorting.binarySearchFromTo(this.elements, key, from, to);
	}
	...
	public boolean removeAll(AbstractLongList other) {
		...
		double N = other.size();
		double M = mySize;
		if ((N + M) * com.github.lightning.internal.bundle.cern.jet.math.Arithmetic.log2(N) < M * N) {
	 ...
	 * Countsort is not always applicable, but if applicable, it usually
	 * outperforms quicksort by a factor of 3-4.
	 * 
	 * <p>
