 ...
 */
package com.github.lightning.internal.bundle.cern.colt.map;

import java.lang.reflect.Array;

import com.github.lightning.internal.bundle.cern.colt.function.LongObjectProcedure;
import com.github.lightning.internal.bundle.cern.colt.function.LongProcedure;
import com.github.lightning.internal.bundle.cern.colt.list.ByteArrayList;
import com.github.lightning.internal.bundle.cern.colt.list.LongArrayList;
import com.github.lightning.internal.bundle.cern.colt.list.ObjectArrayList;
...
public class OpenLongObjectHashMap<T> extends AbstractLongObjectMap<T> {
	 ...
	 */
	public OpenLongObjectHashMap(Class<?> type) {
		this(type, defaultCapacity);
	}
	 ...
	 *             than zero.
	 */
	public OpenLongObjectHashMap(Class<?> type, int initialCapacity) {
		this(type, initialCapacity, defaultMinLoadFactor, defaultMaxLoadFactor);
	}

	/**
	 * Constructs an empty map with
	 * the specified initial capacity and the specified minimum and maximum load
	 * factor.
	 * 
	 * @param initialCapacity
	 ...
	 *             .
	 */
	public OpenLongObjectHashMap(Class<?> type, int initialCapacity, double minLoadFactor, double maxLoadFactor) {
		this.type = type;
		setUp(type, initialCapacity, minLoadFactor, maxLoadFactor);
	}
	 ...
	 * All methods which <i>can</i> be expressed in terms of this method (most
	 * methods can) <i>must guarantee</i> to use the <i>same</i> order defined
	 * by this method, even if it is no particular order.
	 * This is necessary so that, for example, methods <tt>keys</tt> and
	 ...
	 *         Therefore, if the returned index < 0, then it is already
	 *         contained at slot -index-1.
	 *         If the returned index >= 0, then it is NOT already contained and
	 ...
	 * It is often a good idea to first check with
	 * {@link #containsValue(Object)} whether there exists an association from a
	 * key to this value.
	 * Search order is guaranteed to be <i>identical</i> to the order used by
	 * method {@link #forEachKey(LongProcedure)}.
	 * 
	 * @param value
	 ...
	 * number of keys in the receiver exceeds the high water mark or falls below
	 * the low water mark.
	 */
	protected void rehash(int newCapacity) {
		...
		T newValues[] = (T[]) Array.newInstance(type, newCapacity);
	 ...
	 *             .
	 */
	protected void setUp(Class<?> type, int initialCapacity, double minLoadFactor, double maxLoadFactor) {
	 ...
	 * size. Releases any superfluous internal memory. An application can use
