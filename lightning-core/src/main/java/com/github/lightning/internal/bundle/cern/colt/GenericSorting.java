...
import com.github.lightning.internal.bundle.cern.colt.function.IntComparator;

/**
 * Generically sorts arbitrary shaped data (for example multiple arrays, 1,2 or
 * 3-d matrices, and so on) using a
 ...
 * amounts
 * of temporary memory. Alternative 3 involves more swapping, more polymorphic
 * message dispatches, no copying and does not need any temporary memory.
 * <p>
 ...
 * user provided {@link com.github.lightning.internal.bundle.cern.colt.Swapper}
 ...
 * {@link com.github.lightning.internal.bundle.cern.colt.Sorting} handle
 * floating point numbers in special ways to guarantee that NaN's are swapped to
 * the end and -0.0 comes before 0.0. Methods delegating to comparators cannot
 * do this. They rely on the comparator. Thus, if such boundary cases are an
 * issue for the application at hand, comparators explicitly need to implement
 * -0.0 and NaN aware comparisons. Remember: <tt>-0.0 < 0.0 == false</tt>,
...
public class GenericSorting extends Object {
	 ...
	 * range. The initial ranges are <code>[first, middle)</code> and
	 ...
	 * exception for any indexes <tt>a</tt> and <tt>b</tt> in the range).
	 * <p>
	 * 
	...
	public static void mergeSort(int fromIndex, int toIndex, IntComparator c, Swapper swapper) {
		 ...
		 * Given only a comparator and swapper we do not know how to copy and
		 * move elements from/to temporary arrays.
		 * Hence, in contrast to the JDK mergesorts this is an "in-place"
		 * mergesort, i.e. does not allocate any temporary arrays.
		 * A non-inplace mergesort would perhaps be faster in most cases, but
		 * would require non-intuitive delegate objects...
		 */
