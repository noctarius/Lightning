...
import com.github.lightning.internal.bundle.cern.colt.function.CharComparator;
...
import com.github.lightning.internal.bundle.cern.colt.function.ShortComparator;

/**
 * Quicksorts, mergesorts and binary searches; complements
 * <tt>java.util.Arrays</tt>.
 * Contains, for example, the quicksort on Comparators and Comparables, which
 * are still missing in <tt>java.util.Arrays</tt> of JDK 1.2.
 * Also provides mergesorts for types not supported in <tt>java.util.Arrays</tt>
 * , as well as a couple of other methods for primitive arrays.
...
public class Sorting extends Object {
	 ...
	 * equal to the specified key, there is no guarantee which of the multiple
	 * elements
	 * will be found.
	 * 
	 * @param list
	...
	private static void mergeSort2(double a[], int fromIndex, int toIndex) {
		...
		if (numNegZeros != 0) {
			int j = new com.github.lightning.internal.bundle.cern.colt.list.DoubleArrayList(a).binarySearchFromTo(0.0d, fromIndex, n - 1); // posn
																																			// of
																																			// ANY
																																			// zero
			do {
	...
	private static void mergeSort2(float a[], int fromIndex, int toIndex) {
		...
		// Postprocessing phase: change 0.0's to -0.0's as required
		if (numNegZeros != 0) {
			int j = new com.github.lightning.internal.bundle.cern.colt.list.FloatArrayList(a).binarySearchFromTo(0.0f, fromIndex, n - 1); // posn
																																			// of
																																			// ANY
																																			// zero
			do {
